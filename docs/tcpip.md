...
[画像]
## UDP
- 転送中に発生したsocketの破損等を検知し破棄する。<br>
→ それで問題ない場合、またはそれを考慮して利用される（HTTP/3）。<br>
→ 送信内容を保証しないためバッファがいらないのでコストが低い。<br>
...
### アルゴリズム
準備
* 【サーバー側】秘密鍵、公開鍵を生成する（server.key）。
* 【サーバー側】サーバー証明書を取得するため、公開鍵とサーバーの情報（ドメイン名、組織名など）を含んだ証明書署名要求（CSR）を作成する。
* 【サーバー側】CSRをCAに提出し、サーバー証明書（SSL証明書）を購入する。
  ```
...
ハンドシェイク
* 【クライアント側】接続要求する。
* 【サーバー側】クライアントにサーバー証明書を送信する。
* 【クライアント側】サーバー証明書が改竄されていないか、ルート証明書（ブラウザにインストールされているCAの公開鍵）で検証する。
* 【クライアント側】検証の中で、サーバー証明書から公開鍵を抽出する。（証明書の中に含まれている公開鍵部分を取り出すというイメージ）
* 【クライアント側】プリマスターシークレット（共通鍵（セッションキー）の元となる乱数）を生成し公開鍵で暗号化する。
* 【クライアント側】サーバーへプリマスターシークレットを送信する。
* 【サーバー側】プリマスターシークレットを秘密鍵で復号化し、共通鍵を取得する。
* 【クライアント、サーバー側】プリマスターシークレットから共通鍵を生成する。
...
## CDN, LB
### CDN, LBなど前段サーバーの役割
1. 外部から直接バックエンドサーバーにアクセスさせないことでセキュリティの向上につながる。
2. 外部からは前段サーバーのIPしか見えないので、後ろで分散処理するためにバックエンドサーバーを増やしやすい。
3. TLS終端を担う。TLSの暗号化・複合化の負荷が高いため、バックエンドサーバーの負荷にならないようにする。

![IMG_0315](https://github.com/kuwabaray/note/assets/49186880/6cbb71f4-700b-4c4c-a0b6-c8086f98f9c4)

>WAF（Web アプリケーションファイアウォール）とは、Web アプリケーションの通信をフィルター、監視、ブロックするためのソフトウェアまたは、ハードウェアのセキュリティ対策です
(https://aws.amazon.com/jp/waf/what-is-waf/ より引用)

### CDN (Content Delivery Network) 
CDNは通信時間短縮のため各リージョンに配置されたリバースプロキシサーバー。

#### CDN/リバースプロキシサーバーの主な役割
1. HTTPレスポンスをgzip圧縮することでネットワーク上での送信を高速化する。
2. キャッシング
3. アクセスログ出力
```
＊バックエンドサーバーの負荷を軽減・高速化することをWebアクセラレーションという
```
### LB (Load Balancer)
#### LBの主な役割
1. リクエストの分散
2. ヘルスチェック
### NGINX
NGINXはリバースプロキシ、LB、WAFの機能を兼ね備えたミドルウェア（=アプリケーション間通信のためのソフトウェア）
![IMG_0314](https://github.com/kuwabaray/note/assets/49186880/363944fe-b2a0-46c9-b7e4-9b0016a0c90d)
...
[3] [Socket.io How it works](https://socket.io/docs/v3/how-it-works/)<br>
...
[5] [WebSocket Protocol RFC 6455 — The WebSocket Protocol](https://triple-underscore.github.io/RFC6455-ja.html)<br>
[6] [NGINX リバースプロキシとロードバランサーの違いとは？](https://www.nginx.co.jp/resources/glossary/reverse-proxy-vs-load-balancer/)<br>
