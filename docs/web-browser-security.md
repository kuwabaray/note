...
HTTPSはTLSによって暗号化して送受信されるHTTP通信プロトコル。

## SSL/TLS (TLS)
SSLの後継がTLSで総じてSSL/TLSと呼ばれることもある。
...
## リバースプロキシサーバー
TLSターミネーション（TLS終端）がWebサーバー/アプリケーションサーバーの場合、TLSの暗号化・複合化の負荷が高いためにレスポンス速度の低下など障害になり得る。
そこで必要になるのがリバースプロキシサーバー。

### リバースプロキシサーバーの役割
1. 外部から直接サーバーにアクセスさせないことでセキュリティの向上につながる。
2. 外部からはリバースプロキシのIPしか見えないので、リーバープロキシの後ろで、分散処理するためにサーバーを増やしやすい。
3. サーバーの前段に配置されTLS終端を担う。＊1
4. HTTPレスポンスをgzip圧縮することでネットワーク上での送信を高速化する。＊2
5. キャッシング ＊3
```
*1,2,3 このようにサーバーの負荷を軽減したり高速化することをWebアクセラレーションという
```
## Cookie
